...
namespace Algar.Hours.Application.DataBase.Aprobador.Commands.Consult
{
    ...
    public class ConsultAprobadorCommand : IConsultAprobadorCommand
    {
        ...
        public async Task<List<AprobadorUsuarioModel>> Execute(int nivel, Guid idPais)
        {
            if (nivel == 3)//sistema
            {
               ...
               .Where(x => x.UserEntity.IdUser == Guid.Parse("53765c41-411f-4add-9034-7debaf04f276") && x.UserEntity.CountryEntityId == idPais).ToList();
            ...
            else
            {
                var entity = _dataBaseService.AprobadorUsuario
                               .Include(e => e.UserEntity)
                               .Include(e => e.Aprobador)
                               .Where(x => x.Aprobador.Nivel == nivel && x.UserEntity.CountryEntityId == idPais).ToList(); 
